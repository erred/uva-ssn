    ...
    public void updateInterpolationForScreenPosition() {
        if (this.containingScrollView != null) {
            if (this.containingScrollView.getChildCount() != 0) {
                ...
                if (top < 0) {
                    this.materialShapeDrawable.setInterpolation(Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(1.0f, (((float) top) / ((float) height)) + 1.0f)));
                    this.shapedView.invalidate();
                } else {
                    int i = top + height;
                    if (i > height2) {
                        this.materialShapeDrawable.setInterpolation(Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(1.0f, 1.0f - (((float) (i - height2)) / ((float) height)))));
                        this.shapedView.invalidate();
